...
**********************
* 1�-If the Quantity for a product is below 5 we need to make the factory produce new Instruments.
* 2�-If it's above 5 we cool.
**********************/
control_Instrument_Stock(Frame,Nome,Ref,Data_Fab,Preco,Pecas,Quantidade):-

  (Quantidade =< 5 -> produce_Instrument(Nome),
    get_date(D),
    genmsg(Nome+'Instrumento em pouca quantidade, f�brica a produzir',Quantidade,D);
...
***********************
* 1�-Checks how many Instruments we can produce in a factory and displays that number.
* 2�-Asks how many we want to produce.
* 3�-Checks if it's possible to cover that request.
* 4�-Makes the necessery steps to make that product
***********************/

produce_Instrument(Nome):-
  get_Possible_stock(Nome,Stock),
  Stock =:= 0,
  (write('Poss�vel produzir '),write(Stock),write(Nome),nl,
  ...
  (Produce =< Stock ->
    ...
    Produce > Stock -> write('N�o � possivel produzir!');

    Produce =:= 0 -> write('Nada foi produzido')
  ).
...
control_Factory_Stock(F,Nome,_,Cap,Actual_Stock):-
  ...
  get_date(D),
  genmsg('A chegar ao limite de capacidade da f�brica', Difer, D).
